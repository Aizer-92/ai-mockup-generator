...
class BatchProcessor:
    ...
    def _create_collection_analysis_prompt(self, product_color: str, collection_style: str, 
        ...
        return f"""🚨 КРИТИЧЕСКИ ВАЖНО: Анализируй ТОЧНО ТЕ ТОВАРЫ, которые показаны на изображениях. НЕ МЕНЯЙ ТИП ТОВАРА! 🚨
...
ЗАДАЧА:
1. ВНИМАТЕЛЬНО изучи каждый товар на изображениях - определи ЧТО ЭТО ЗА ТОВАР
2. Для каждого конкретного товара выбери ПРОСТОЕ И РЕГУЛЯРНОЕ нанесение логотипа:
...
3. Определи оптимальное размещение логотипа для КАЖДОГО КОНКРЕТНОГО товара
4. Выбери подходящий размер логотипа
5. Определи лучший ракурс для каждого товара
6. Создай индивидуальный промпт для каждого товара
7. Убедись, что все товары выглядят как единая коллекция
...
- В custom_prompt укажи конкретный тип товара из изображения
...
- custom_prompt: дополнительные детали с указанием конкретного типа товара (БЕЗ текста "Товар X коллекция")
- reasoning: объяснение выбора для этого товара с указанием типа товара"""
