...
# Автоматическая настройка сервера с SSH ключом
# Запуск: ./auto_setup_with_ssh.sh

echo "🚀 Автоматическая настройка сервера с SSH ключом"
echo "================================================"
...
if ! ssh -o ConnectTimeout=10 search.headcorn.pro "echo 'SSH подключение работает'" > /dev/null 2>&1; then
    echo "❌ SSH подключение не работает"
    echo "Проверьте настройку SSH ключа:"
    echo "   ./setup_ssh_key.sh"
    exit 1
fi
