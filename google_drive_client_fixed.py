"""
Улучшенный модуль для работы с Google Drive API
Поддерживает как Desktop, так и Web application credentials
"""
import os
import io
import json
...
class GoogleDriveClientFixed:
    """Улучшенный клиент для работы с Google Drive API"""
    ...
    def detect_credentials_type(self) -> str:
        """Определяет тип credentials (desktop или web)"""
        try:
            ...
            print(f"Ошибка определения типа credentials: {e}")
            return 'unknown'
    
    def authenticate(self) -> bool:
        """
        Аутентификация в Google Drive API с поддержкой разных типов credentials
        ...
        try:
            ...
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token:
                    ...
                    self.credentials_type = self.detect_credentials_type()
                    print(f"Тип credentials: {self.credentials_type}")
                    
                    if self.credentials_type == 'desktop':
                        # Desktop application flow
                        flow = InstalledAppFlow.from_client_secrets_file(
                            self.credentials_file, self.SCOPES)
                        creds = flow.run_local_server(port=0)
                    elif self.credentials_type == 'web':
                        ...
                        print("Неизвестный тип credentials. Поддерживаются только 'desktop' и 'web'")
                        return False
...
def get_drive_client_fixed() -> Optional[GoogleDriveClientFixed]:
    """
    Получает настроенный клиент Google Drive с улучшенной аутентификацией
    
    Returns:
        GoogleDriveClientFixed: Настроенный клиент или None
    """
    try:
        client = GoogleDriveClientFixed()
