...
def generate_creative_concepts(brandbook_files):
    ...
    try:
        ...
        # Теперь создаем концепции с учетом анализа товара
        st.info("Создаем концепции на основе детального анализа товара...")
        
        # Обновляем основной промпт с учетом анализа товара
        if product_analysis:
            analysis_prompt = f"""
            Ты - эксперт по мерчендайзингу и дизайну товаров. Проанализируй предоставленные файлы брендбука и создай 5 уникальных концепций для товара.
